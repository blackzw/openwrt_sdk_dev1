...
if not net then
	luci.http.redirect(luci.dispatcher.build_url("admin/basicsettings/lan/lansettings"))
	return
end
...
if m:formvalue("cbid.network.%s._switch" % net:name()) then
	...
	if proto then
		...
		-- reload page
		luci.http.redirect(luci.dispatcher.build_url("admin/basicsettings/lan/lansettings", arg[1]))
		return
	end
end
