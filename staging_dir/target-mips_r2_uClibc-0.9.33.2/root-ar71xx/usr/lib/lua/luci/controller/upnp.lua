...
--[[	if not nixio.fs.access("/etc/config/upnpd") then
	...
	page = entry({"admin", "advancedsettings", "nat", "upnp"}, cbi("upnp/upnp"), _("UPnP Settings"),3)
	page.dependent = true

	entry({"admin", "advancedsettings", "nat", "upnp", "status"}, call("act_status")).leaf = true
	entry({"admin", "advancedsettings", "nat", "upnp" ,"delete"}, call("act_delete")).leaf = true  ]]--
end
