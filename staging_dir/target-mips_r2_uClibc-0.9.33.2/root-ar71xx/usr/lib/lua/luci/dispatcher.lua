...
function authenticator.htmlauth(validator, accs, default)
	...
	local logstate = luci.sauth.loginstate()
	if user and validator(user, pass) and logstate ~= 1 and user == "admin" then
		return user
	end
	...
	require("luci.template")
	context.path = {}
	luci.template.render("sysauth", {duser=default, fuser=user,fstate=logstate})
	return false
...
function dispatch(request)
	...
	if track.sysauth then
		...
		--[[local logstate = sauth.loginstate()
		if logstate == 1 then
			luci.http.write_json(logstate)
		end--]]
		...
		if not util.contains(accs, user) then
			if authen then
				...
				--local tmp = luci.sys.exec("ls /tmp/luci-sessions/")
				--local logstate = sauth.loginstate()
				...
				if not user or not util.contains(accs, user) then
					...
					local lsid
					local logined = luci.http.getcookie("Login")
					if logined == nil or logined == "" then 
							...
							local logined = sess or luci.http.getcookie("Login")
							logined = luci.http.getcookie("Login")
							if not sess then
								ok = sauth.read(logined)
								sess = logined
								lsid = logined
								if ok == nil then
									luci.http.redirect(build_url())
									luci.http.write_json("ok is nil")
									return 
								end
								local token = ok.token
								stoks = token
								ctx.urltoken.stok = token
							end
					end
					luci.http.header("Set-Cookie", "sysauth=" .. lsid.."; path="..build_url())
					ctx.authsession = lsid
					ctx.authuser = user
					luci.http.redirect(build_url())
				end
			else
