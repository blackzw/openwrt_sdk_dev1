...
mac80211_hostapd_setup_base() {
	...
	[ "$channel" = auto-2.4G ] && {
		channel=$(iw phy "$phy" info | \
			sed -ne '/2[0-9][0-9][0-9][[:space:]]MHz/ { /disabled\|passive\|radar/d; s/.*\[//; s/\].*//; p; q }')
		}
	[ "$channel" = auto-5G ] && {
		channel=$(iw phy "$phy" info | \
			sed -ne '/5[0-9][0-9][0-9][[:space:]]MHz/ { /disabled\|passive\|radar/d; s/.*\[//; s/\].*//; p; q }')
		}
		config_get htmode "$device" htmode
		if [ $htmode != "HT20" ]; then
			if [ $channel -ge 5 ]; then
				if [ $channel -le 13 ]; then
				config_set "$device" htmode "HT40-"
				fi
			fi
			if [ $channel -ge 1 ]; then
				if [ $channel -le 7 ]; then
					config_set "$device" htmode "HT40+"
				fi
			fi
			case $channel in
				40|48|56|64|153|161) config_set "$device" htmode "HT40-" 
				;;
				36|44|52|60|149|157|165) config_set "$device" htmode "HT40+" 
				;;
				*);;
			esac 
		fi
...
enable_mac80211() {
	...
	[ "$channel" = "auto-2.4G" -o "$channel" = "0" ] || {
		fixed=1
	}
	
	[ "$channel" = "auto-5G" -o "$channel" = "0" ] || {
		fixed=1
	}
...
detect_mac80211() {
	passwd=$(mmi --wlpasswd -custom)
	ssid_24g=$(mmi --wlssid_24g -custom)
	ssid_5g=$(mmi --wlssid_5g -custom)
	devidx=0
...
config wifi-device  radio$devidx
	...
	option wps_hotplug '1'
	option channel 'auto-2.4G'
	option country 'CN'
	option current_band '1'
	option 24g_channel 'auto-2.4G'
	option 5g_channel 'auto-5G'
	option 24g_hwmode 11${mode_11n}${mode_band}
	option 5g_hwmode '11na'
$dev_id
$ht_capab
...
config wifi-iface
	...
	option mode     ap
	option band     '2.4'
	option ssid    '$ssid_24g'
	option encryption 'psk2'
	option key '$passwd' 
	option wep_key '1234567890'
	option wpa_key '$passwd'
	option isolate '1'
	option wps_pushbutton '1'
	option wps_ap_pin '0'
	option ap_pin '12345670'
	option disabled '0'
	option hidden '0'
	option fixed '0'
	option max_num_sta '20'
...
config wifi-iface
	...
	option mode     ap
	option band     '2.4'
	option ssid    'H850-MM100'
	option encryption 'psk+mixed+ccmp'
	option key 'H850-MM100'
	option isolate '1'
	option wps_pushbutton '1'
	option wps_ap_pin '0'
	option ap_pin '12345670'
