hostapd_set_bss_options() {
	...
	case "$enc" in
		*tkip+aes|*tkip+ccmp|*aes+tkip|*ccmp+tkip|psk|psk2) crypto="CCMP TKIP";;
	...
	config_get_bool wps_ap_pin "$vif" wps_ap_pin 0

	config_get config_methods "$vif" wps_config
	[ "$wps_pbc" -gt 0 ] && append config_methods push_button
	[ "$wps_ap_pin" -gt 0 ] && append config_methods wps_label
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		if [ "$crypto" = "TKIP" ]
		then
			append "$var" "wps_state=0" "$N"  
		else
			append "$var" "wps_state=2" "$N"
		fi
		
		[ "$wps_ap_pin" -eq 1 ] && append "$var" "ap_pin=$ap_pin" "$N"
		# [ "$wps_ap_pin" -eq 0 ] && append "$var" "ap_setup_locked=1" "$N"
		append "$var" "device_type=$device_type" "$N"
		append "$var" "device_name=$device_name" "$N"
		append "$var" "manufacturer=$manufacturer" "$N"
		append "$var" "config_methods=$config_methods" "$N"
		append "$var" "max_num_sta=20" "$N"
	}
...
hostapd_setup_vif() {
	...
	{[ "$channel" = auto-2.4G ] || [ "$channel" = auto-5G ]} && channel=
