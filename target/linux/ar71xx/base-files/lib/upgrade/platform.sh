...
restore_config_check(){ 
	rm -fr /tmp/restore_check                                       
        if [ -f "/tmp/$1" ];then                      
                if [ ! -d "/tmp/backup_h850" ];then           
                        mkdir /tmp/backup_h850             
               fi                                         
                                                                              
                tar -zxf /tmp/$1 -C /tmp/backup_h850
                                                                               
                if [ -d "/tmp/backup_h850/etc" ] && [  -d "/tmp/backup_h850/usr" ]; then
		 echo "true" > /tmp/restore_check
		 rm -fr /tmp/$1
		 cp  -fr /tmp/backup_h850/* /
		 return 0                                                                                                                             
                fi                                                                                                                                    
        fi  
        rm -rf /tmp/backup_h850
        return 1                                                                            
} 
